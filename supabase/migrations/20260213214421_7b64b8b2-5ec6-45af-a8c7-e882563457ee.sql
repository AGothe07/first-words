...
BEGIN
  ...
  -- ============ CURRENT_MONTH (with persons + subs in top_expenses/top_incomes) ============
  ...
  FROM (
    SELECT sub.cat_name, jsonb_agg(jsonb_build_object(
      'm', sub.month, 'v', sub.total,
      'subs', sub.subs_data,
      'persons', sub.persons_data
    ) ORDER BY sub.month DESC) as monthly_data
    FROM (
      SELECT c.name as cat_name, c.id as cat_id, to_char(t.date::date, 'YYYY-MM') as month, round(SUM(t.amount), 2) as total,
        COALESCE((
          SELECT jsonb_agg(jsonb_build_object('name', sc.name, 'total', round(ss.total, 2), 'count', ss.cnt))
          FROM (
            SELECT t2.subcategory_id, SUM(t2.amount) as total, COUNT(*) as cnt
            ...
            WHERE t2.user_id = p_user_id AND t2.category_id = c.id
              AND to_char(t2.date::date, 'YYYY-MM') = to_char(t.date::date, 'YYYY-MM')
        ...
        COALESCE((
          ...
          FROM (
            SELECT t3.person_id, SUM(t3.amount) as total, COUNT(*) as cnt
            FROM public.transactions t3
            WHERE t3.user_id = p_user_id AND t3.category_id = c.id
              AND to_char(t3.date::date, 'YYYY-MM') = to_char(t.date::date, 'YYYY-MM')
            GROUP BY t3.person_id
          ) pp
          JOIN public.persons pe ON pe.id = pp.person_id
        ), '[]'::jsonb) as persons_data
      FROM public.transactions t
      JOIN public.categories c ON c.id = t.category_id
      WHERE t.user_id = p_user_id AND t.date::date >= (now() - interval '6 months')::date
      GROUP BY c.name, c.id, to_char(t.date::date, 'YYYY-MM')
    ) sub
